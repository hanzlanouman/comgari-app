...
const Profile = () => {
  ...
  const updateProfilePic = async () => {
    try {
      const { isSuccess, error, result } = await pickImage(false, {
        quality: 1,
        aspect: [1, 1],
      });
      if (!isSuccess) {
          ...
          {expandedSection === "detele" && (
            <View className="border-t border-light p-4">
              ...
              <Text className="text-sm font-ManropeLight text-dark mb-4 mt-1">
                ...
                deleted. All you subscribed to will be canceled. You will not be
                able to recover your account or subscriptions after deletion.
              </Text>
              <View className="pb-4">
                <InputField
